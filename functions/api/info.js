...
function decryptTool77Url(encryptedUrl) {
  ...
  try {
    ...
    // Use atob in Cloudflare Worker environment instead of Node's Buffer
...
async function fetchTool77Info(url) {
  ...
  if (data.code !== 'success' || !data.data) {
    throw new Error(data.message || 'Request failed on engine');
  }

  const videoData = normalizePayload(data.data);
  if (!videoData) {
    throw new Error('Failed to parse downloader engine payload');
  }
  const formats = [];
...
// Cloudflare Pages FunctiononRequest Handler
